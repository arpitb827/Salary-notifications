...
class department_budget_report(osv.osv_memory):
    ...
    def report_get(self,cr,uid,ids,context=None):
        ...
        ws.col(2).width = 8000
        ws.write(0,4,'New Salary Amount',style_header)
        ws.col(3).width = 8000
        ws.write(0,3,'Old Salary Amount',style_header)
        ws.col(4).width = 8000
...
#        ws.col(9).width = 4400
        ...
        if this.dept_id:
            ...
            cr.execute("select emp.id from hr_employee as emp left join resource_resource \
             as res on (emp.resource_id=res.id) where res.active=True order by emp.department_id, (substring(emp.sinid, '^[0-9]+'))::int ,substring(emp.sinid, '[^0-9_].*$')")
            temp = cr.fetchall()
            for data in temp:
                if len(data)>0 and data[0] != None:
                    emp_ids.append(data[0])
            ...
            if dept_dict.has_key(str(each.department_id.id)):
                if each.department_id:
                    ...
                    if each.daily:
                        ...
                        old_salary = each.new_salary * working_day
                    else:
                        salary = each.salary
                        old_salary = each.new_salary
                    ws.write(i,4, salary)
                    #if pay_data and pay_data.employee_id.id == each.id:
                    ws.write(i,3,old_salary)
                    #else:
                        #ws.write(i,4, 0.0)
                    total += salary
                    grand +=  salary
...
#                     else:
                    pay_total += old_salary
                    pay_grand += old_salary
                ...
                if flag:
                    ...
                    ws.write(i,4, total,style_header)
                    if pay_total:
                        ws.write(i,3,pay_total,style_header)
                    else:
                        ws.write(i,3, 0.0,style_header)
                    if budget:
...
#                     else:
        ...
        ws.write(i+1,4, total,style_header)
        ws.write(i+3,2, 'Grand Total',style_header)
        ws.write(i+3,4, grand,style_header)
        
        ws.write(i+1,3, pay_total,style_header)
        ws.write(i+3,3, pay_grand,style_header)
