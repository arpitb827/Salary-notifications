...
#                print "year_id.............",year_id.name,type(year_id.name)
    ...
    def calculate_department_salary(self, cr, uid, ids, context=None):
        ...
        for each in self.browse(cr, uid, ids):
            ...
            for data in temp:
                ...
                print "<========COUNT==========>",count,"<====================   NEW DATA IS CREATED   ================>",ser_id
    ...
    def calculate_payment(self, cr, uid, ids, context=None):
        ...
        month = off_day = sunday = count_down = 0
        for line in self.browse(cr, uid, ids):
            if line.type:
                emp_ids = emp_obj.search(cr, uid, [('active','=',True),('shift_lines','!=',False),('type','=',line.type)])
            else:
                emp_ids = emp_obj.search(cr, uid, [('active','=',True),('shift_lines','!=',False),('type','in',['Metal','Wood'])])
...
#                cr.execute("select id from attendance_timing where employee_id ='"+str(val.id)+"' and DATE_PART('MONTH',name)='"+str(line.month.month)+"' and status <> 'D_Miss_Punch'")
                ...
                if line.salary_type == 'Kharcha':
                    if val.type in ['Wood','Metal']:
                        if val.id in [9654,11250,9658,1367,11244,9700,9695,9679,9853,9897,10817,10149,9802,14824,15588]:
                            cr.execute("select id from attendance_timing where employee_id ='"+str(val.id)+"' and DATE_PART('MONTH',name)='"+str(line.month.month)+"' and name <= '"+str(end_date)+"' and (status is null or status in ('A_OK','B_Reduced'))")
                        else:    
                            cr.execute("select id from attendance_timing where employee_id ='"+str(val.id)+"' and DATE_PART('MONTH',name)='"+str(line.month.month)+"' and name <= '"+str(end_date)+"' and (status is null or status in ('A_OK','B_Reduced')) and dept_status='OK' ")
                ...
                for att in att_obj.browse(cr, uid, att_list):
                    ...
                    for dept in dept_obj.browse(cr, uid, dept_obj.search(cr, uid, [('att_date','=',att.name),('employee_id','=',val.id)])):
                        ...
                        if daily_amt2 > 0:
                            ...
                            count_down += 1
                            print "<----------count--------->",count_down,"<------------EMPLOYEE-------->",val.sinid,"<-------SALARY CALCULATED TRANSFER------->",total_amount
                        else:    
                            ...
                            'total_amount':total_amount,'month':line.month.month,'state':'Draft','salary_type':line.salary_type,'att_type':dept.type})
                            count_down += 1
                            print "<----------count--------->",count_down,"<------------EMPLOYEE-------->",val.sinid,"<-------SALARY CALCULATED SALARY------->",total_amount
        return res
