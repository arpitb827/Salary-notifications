...
class department_budget_report(osv.osv_memory):
    ...
    def account_budget(self,cr,uid,ids,context=None):
        ...
        ws.col(1).width = 8000
        ws.write(0,2,'No. Of Bud. Emp',style_header)
        ws.col(2).width = 8000
        ws.write(0,3,'No. Of Sal. Emp',style_header)
        ws.col(3).width = 8000
        ws.write(0,4,'Allocated Budget',style_header)
        ws.col(4).width = 8000
        ws.write(0,5,'Salary Amount',style_header)
        ws.col(5).width = 5000
        ws.write(0,6,month_name,style_header)
        ws.col(6).width = 8000
        ws.write(0,7,'Difference',style_header)
        ws.col(7).width = 8000
...
#        ws.col(9).width = 4400
        ...
        if this.dept_id:
            cr.execute("select emp.id from hr_employee as emp left join resource_resource " \
             ...
             "emp.department_id is not null and res.active=True order by dept.dept_code, " \
            ...
            cr.execute("select emp.id from hr_employee as emp left join resource_resource " \
             ...
             "where res.active=True order by dept.dept_code,(substring(emp.sinid, '^[0-9]+'))::int ,substring(emp.sinid, '[^0-9_].*$')")
            temp = cr.fetchall()
            for data in temp:
                if len(data)>0 and data[0] != None:
                    emp_ids.append(data[0])
        ...
        for each in emp_obj.browse(cr, uid, emp_ids):
            ...
            if dept_dict.has_key(str(each.department_id.id)):
                ...
                if flag:
                    i -= 1
                    ws.write(i,5, total)
                    ws.write(i,6, pay_total)
                    ws.write(i,2, count_emp)
                    ws.write(i,3, sal_emp)
                    if budget:
                        diff = float(budget) - pay_total 
                    else:
                        diff = pay_total
                    if budget and diff > 0:
                        ws.write(i,7, diff,style_header1)
                    elif budget and diff < 0:
                        ws.write(i,7, diff,style_header2)
                    else:
                        ws.write(i,7, diff,style_header2)
                    flag = False
                    count_emp = 0
...
#                     ws.write(i,4, 0.0)
        ...
        ws.write(i+3,2, total_emp,style_header)
        ws.write(i+3,3, total_sal_emp,style_header)
        ws.write(i+3,4, total_budget,style_header)
        ws.write(i+3,5, grand,style_header)
        ws.write(i+3,6, pay_grand,style_header)
        
        diff_grand = total_budget - pay_grand
        
        ws.write(i+3,7, diff_grand,style_header1)
    ...
    def detail_budget(self,cr,uid,ids,context=None):
        ...
        ws.write(0,3,'Designation',style_header)
        ws.col(3).width = 8000
        ws.write(0,4,'Working Month',style_header)
        ws.col(4).width = 5000
...
#        ws.col(9).width = 4400
        ...
        if this.dept_id:
            cr.execute("select emp.id from hr_employee as emp left join resource_resource "\
             ...
             "res.active=True order by emp.department_id, (substring(emp.sinid, '^[0-9]+'))::int ,substring(emp.sinid, '[^0-9_].*$')")
            temp = cr.fetchall()
            for data in temp:
                if len(data)>0 and data[0] != None:
                    emp_ids.append(data[0])
        ...
        for each in emp_obj.browse(cr, uid, emp_ids):
            ...
            if dept_dict.has_key(str(each.department_id.id)):
                if each.department_id:
                    ...
                    name = '[' + str(each.sinid) +'] '+ str(each.name)
                    ws.write(i,2, name)
                    ws.write(i,3, each.designation_id and each.designation_id.name or '')
                    ws.write(i,4, str(tolal_months) + ' Month')
                    ...
                    ws.write(i,5, salary)
                    if pay_data and pay_data.employee_id.id == each.id:
                        ws.write(i,6,pay_data.total_amount or 0.0)
                    else:
                        ws.write(i,6, 0.0)
                    total += salary
                    grand +=  salary
                ...
                if flag:
                    ...
                    ws.write(i,5, total,style_header)
                    if pay_total:
                        ws.write(i,6,pay_total,style_header)
                    else:
                        ws.write(i,6, 0.0,style_header)
                    if budget:
        ...
        ws.write(i+1,5, total,style_header)
        ws.write(i+3,2, 'Grand Total',style_header)
        ws.write(i+3,5, grand,style_header)
        ws.write(i+1,6, pay_total,style_header)
        ws.write(i+3,6, pay_grand,style_header)
        
        diff_pay = pay_total - total
        diff_grand = pay_grand - grand
        
        ws.write(i+1,7, diff_pay,style_header1)
        ws.write(i+3,7, diff_grand,style_header1)
        f = cStringIO.StringIO()
        wb.save(f)
        out=base64.encodestring(f.getvalue())
