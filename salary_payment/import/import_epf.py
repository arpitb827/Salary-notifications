...
def run(filename=None, host='localhost', db=None, login=None, passw=None):
    ...
    paid_obj=conn.get_model("payment.management.tds")
    ...
    for row in data:
        ...
        if not emp_ids:
            emp = sinid,row[name],row[amt]
            remaining_emp_list.append(list(emp))
            continue
        
        print "===============row==================",sinid,row
        print "===============row[date]==================",sinid,row,row[date]
        print "===============row[amt]==================",sinid,row,row[amt]
        print "===============emp_ids[0]==================",sinid,row,emp_ids[0]
        
#        try:
        
        paid_id=paid_obj.create({'name':row[date],'tds':row[amt],
                                      'employee_id':emp_ids[0],'month_id':2,'year_id':2})
...
#            
#            writer = csv.writer(open("/tmp/epf.csv", "wb"))
