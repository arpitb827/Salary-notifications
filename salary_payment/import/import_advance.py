...
def run(filename=None, host='localhost', db=None, login=None, passw=None):
    ...
    for row in data:
        ...
        if not emp_ids:
            emp = sinid,row[name],row[amt],row[balance]
            remaining_emp_list.append(list(emp))
            continue
        
        try:
        
            paid_id=paid_obj.create({'name':row[date],'advance_date':row[date],'total_amount':row[amt],
            'balance_amount':row[balance],'amount_emi':row[balance],'employee_id':emp_ids[0],'month_id':2,'year_id':2})
