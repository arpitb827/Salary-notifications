...
def run(filename=None, host='localhost', db=None, login=None, passw=None):
    ...
    paid_obj=conn.get_model("payment.management.panalty")
    ...
    for row in data:
        ...
        try:
            ...
            paid_id=paid_obj.create({'name':row[date],'penalty_date':row[date],'amount':row[amt],
                                          'employee_id':emp_ids[0],'month_id':2,'year_id':2,'remark':'Import From File'})
            ...
            writer = csv.writer(open("/tmp/penalty.csv", "wb"))
